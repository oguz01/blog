@extends('layouts.sb-admin.app')
@section('title', 'Etiket | ' . env('BLOG_TITLE'))
@section('content')
    <h1 class="h3 mb-0 text-gray-800">Etiket</h1>
    @if (session('success'))
        <div class="alert alert-success" role="alert">
            {{ session('success') }}
        </div>
    @endif
    <a href="{{ route('tag.create') }}" class="btn btn-primary btn-sm mt-2">
        <i class="fas fa-plus"></i> Etiket
        Oluştur</a>
    @if ($tag[0])
        {{-- Table --}}
        <table class="table table-bordered table-striped table-sm table-hover mt-3">
            <thead>
                <tr>
                    <th scope="col">#</th>
                    <th scope="col">Etiket İsmi</th>
                    <th scope="col">Slug</th>
                    <th scope="col">İşlem</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($tag as $item)
                    <tr>
                        <th scope="row">{{ $loop->iteration }}</th>
                        <td>{{ $item->name }}</td>
                        <td>{{ $item->slug }}</td>
                        <td class="align-items-center d-flex justify-content-center">
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <a href="{{ route('tag.edit', $item->id) }}" class="btn btn-primary btn-sm mr-1"><i
                                        class="fas fa-edit"></i> Düzenle</a>
                                <form method="POST" action="{{ route('tag.destroy', $item->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-danger btn-sm" onclick="return confirm('Eminmisin Bu işin Geri Dönüşü Yok Babbbaaaaaa....?')">
                                        <i class="fas fa-trash"></i> Sil
                                    </button>
                                </form>
                            </div>

                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        <div class="d-flex mb-3 justify-content-end">
            {{ $tag->links() }}
        </div>
    @else
        <div class="alert alert-info mt-3 text-center" role="alert">
            Mevcut Etiket Bulunmamaktadır.
        </div>
    @endif
@endsection
